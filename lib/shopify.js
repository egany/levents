...
async function readOneCustomer(params) {
  try {
    ...
    if (
      ...
      body.data.customers.edges.length === 0
    ) {
      result.data = null;
      return result;
    }
 ...
 * @returns {Promise<Levents.Shopify.ReadOneCustomerResult>}
 */
async function readManyCustomers(params) {
  try {
    /**@type {Levents.Shopify.ReadOneCustomerResult} */
    let result = {
      data: null,
    ...
    const { body } = await client.query({
      data: `query {
        customers(first: 20, query: "${params.phone}", reverse: true) {
          edges {
            node {
              id
              createdAt
    ...
    let customers = body?.data?.customers?.edges?.map((n) => n.node);

    if(customers.length) {
      for(let customer of customers) {
        if(customer.metafields.edges) {
          customer.metafields = customer.metafields.edges.map(e => e.node);
        }
        else {
          customer.metafields = []
        }
 ...
 * @returns {Promise<Levents.Shopify.ReadOneCustomerResult>}
 */
async function readManyOrders(params) {
  try {
    let result = {
      data: null,
      errors: [],
    ...
    const { body } = await client.query({
      data: `query {
        orders(first: 250, query:"status:open AND created_at:>2023-12-18", after: ${params.after ? params.after : null}) {
          edges {
            node {
              id
              customer {
    ...
    if (
      !Array.isArray(body?.data?.orders?.edges) ||
      body.data.orders.edges.length === 0
    ) {
      result.data = null;
      return result;
    }

    const orders = body?.data?.orders?.edges?.map((n) => n.node);

    if(body?.data?.pageInfo?.hasNextPage) {
      console.log(`Next Page: ${body?.data?.pageInfo?.hasNextPage}`)
      params.after = body?.data?.pageInfo?.endCursor;
      const nextResult =  await readManyOrders(params);

      if(nextResult.data) {
        orders.push(...nextResult.data);
      }
    }
    ...
    console.error(error);
    throw new Error("Read many orders form Shopify faield");
  }
}
